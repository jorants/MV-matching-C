...
void *_MVInfo_init_file_init_graph ( uint num_vertices, uint num_edges )
{
    Graph *g = Graph_init ( num_vertices );
    return g;
}

void _MVInfo_init_file_add_edge ( void *g, uint i, uint j )
{
    Graph_add_edge ( (Graph*) g, i - 1, j - 1 );
}
...
MVInfo_init_file ( char *filename )
{
    ...
    Graph *g = _DIMACS_read_graph ( fp, &_MVInfo_init_file_init_graph, &_MVInfo_init_file_add_edge, &success, message );
