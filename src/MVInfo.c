...
void VertexInfo_delete(VertexInfo *vi)
{
...
void VertexInfo_reset(VertexInfo *vi)
{
  vi->ddfs_color = none;
...
bool VertexInfo_has_min_level(VertexInfo *vi)
{
  return VertexInfo_min_level(vi) != INFINITY ;
}

uint VertexInfo_min_level(VertexInfo *vi)
{
...
MVInfo *MVInfo_init(Graph *g)
{
  ...
  // mvi->ddfs = DDFSInfo_init(mvi->petals);

  // mvi->oldBridges = EdgeList_init();
...
void MVInfo_print_graphviz(MVInfo *mvi, char *filename)
{
  ...
  fprintf(fp, "lvlnode [shape=plaintext,label=\"stage = %d, level = %d\"];", mvi->stage, mvi->level);
  //fprintf(fp, "")
          ...
          if (j > i)
            {
              ...
              //                                else if (EdgeList_find(mvi->oldBridges, e))
              //                                        fprintf(fp, "label=\"b\",fontsize=\"6\",");
  ...
  // // prints arrow edges denoting predecessors
  // for (i = 0; i < mvi->graph->size; i++)
  // {
  //     itr = mvi->v_info[i]->predecessors->first;
  //     while (itr)
  //     {
  //         uint j = itr->value;
  //         fprintf(fp, "n%03d -- n%03d", i, j);
  ...
  //         fprintf(fp,"arrowhead=vee,dir=forward");
