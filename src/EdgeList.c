...
EdgeListIterator *EdgeListIterator_init(EDGE_LIST_TYPE val)
{
...
void EdgeList_add(EdgeList *lst, EDGE_LIST_TYPE val)
{
	if (EdgeList_is_empty(lst))
	{
...
void EdgeList_push(EdgeList *lst, EDGE_LIST_TYPE val)
{
	if (EdgeList_is_empty(lst))
	{
		lst->first = lst->last = EdgeListIterator_init(val);
...
EDGE_LIST_TYPE EdgeList_pop(EdgeList *lst)
{
...
EDGE_LIST_TYPE EdgeList_top(EdgeList *lst)
{
	assert(!EdgeList_is_empty(lst));
...
EDGE_LIST_TYPE EdgeList_nth(EdgeList *lst, uint n)
{
	EdgeListIterator *itr = lst->first;
	while (n != 0)
	{
...
void EdgeList_append_reverse_aux(EdgeList *to, EdgeListIterator *itr)
{
	if (itr -> next)
