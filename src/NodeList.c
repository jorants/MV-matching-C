...
NodeListIterator *NodeListIterator_init(LIST_TYPE val)
{
...
void NodeList_add(NodeList *lst, LIST_TYPE val)
{
	if (NodeList_is_empty(lst))
	{
...
void NodeList_push(NodeList *lst, LIST_TYPE val)
{
	if (NodeList_is_empty(lst))
	{
		lst->first = lst->last = NodeListIterator_init(val);
...
LIST_TYPE NodeList_pop(NodeList *lst)
{
...
LIST_TYPE NodeList_top(NodeList *lst)
{
	assert(!NodeList_is_empty(lst));
...
LIST_TYPE NodeList_nth(NodeList *lst, uint n)
{
	NodeListIterator *itr = lst->first;
	while (n != 0)
	{
...
void NodeList_append_reverse_aux(NodeList *to, NodeListIterator *itr)
{
	if (itr -> next)
