...
PointerListIterator *PointerListIterator_init(PTR_LIST_TYPE val)
{
...
void PointerList_add(PointerList *lst, PTR_LIST_TYPE val)
{
	if (PointerList_is_empty(lst))
	{
...
void PointerList_push(PointerList *lst, PTR_LIST_TYPE val)
{
	if (PointerList_is_empty(lst))
	{
		lst->first = lst->last = PointerListIterator_init(val);
...
PTR_LIST_TYPE PointerList_pop(PointerList *lst)
{
...
PTR_LIST_TYPE PointerList_top(PointerList *lst)
{
	assert(!PointerList_is_empty(lst));
...
PTR_LIST_TYPE PointerList_nth(PointerList *lst, uint n)
{
	PointerListIterator *itr = lst->first;
	while (n != 0)
	{
...
void PointerList_append_reverse_aux(PointerList *to, PointerListIterator *itr)
{
	if (itr -> next)
